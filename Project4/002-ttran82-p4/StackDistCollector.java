...
public class StackDistCollector{
	...
	public Integer access(int address){
		...
		if(firstOccurence(address)){
			SymbolRecord newRecord = new SymbolRecord(address,time);
			symbolTable.putRecord(address,newRecord);
			pastAccesses.insert(time);
			time++;
			return null;
		}else{
			int oldTime = symbolTable.getRecord(address).getLastAccessTime();
			pastAccesses.insert(time);
		   	pastAccesses.remove(oldTime);	//remove old time sequence
			int toReturn = pastAccesses.countNoSmallerThan(oldTime);
		   	SymbolRecord newAcess = new SymbolRecord(address,time);
		   	symbolTable.putRecord(address,newAcess);
		   	time++;
		return toReturn;}
	}

	private  boolean firstOccurence(int address){
	...
	public static void main(String[] args) throws IOException{
		...
		//	System.out.println(collector.access());
