...
public class OPTCacheSimulator{
	 ...
	 * @param stackDist	priority of address
	...
	public boolean access(int address, int stackDist){
		...
		if(symbolTable.hasSymbol(address)){				//if address is inside table simply update cache
			cache.updatePriority(address,stackDist);
			return true;
		}else if(!symbolTable.hasSymbol(address)&& cache.size()<cacheCapacity){	//if it's not inside symbol and cache is not full add to table and cache
			cache.insert(address,stackDist);
			symbolTable.putRecord(address,stackDist);
			cache.updatePriority(address,stackDist);
			return false;
		}//remove address from cache and reinsert and update with new priority
		int removed = cache.remove();
		symbolTable.removeSymbol(removed);
		symbolTable.putRecord(address,stackDist);
		cache.insert(address,stackDist);
		cache.updatePriority(address,stackDist);
		return false;
...
//		simulator.access(20,5);
//		simulator.cache.print();
		//System.out.println(simulator.cache.print());
